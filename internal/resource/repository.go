...
type Repository interface {
	...
	SaveEmbedding(*EmbeddingResource) error
	GetEmbedding(id string) (*EmbeddingResource, error)
	DeleteEmbedding(id string) (bool, error)
...
func NewResourceRepo(db *sql.DB) (*ResourceRepo, error) {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS embedding_resources (
		resource_id     TEXT PRIMARY KEY,
		name            TEXT,
		base_url        TEXT NOT NULL,
		api_key         TEXT NOT NULL,
		...
		return nil, fmt.Errorf("create embedding_resources table: %w", err)
	}
	if err := ensureColumn(db, "embedding_resources", "max_concurrency", "INTEGER"); err != nil {
		return nil, fmt.Errorf("migrate embedding_resources table: %w", err)
	}
...
func (r *ResourceRepo) SaveEmbedding(res *EmbeddingResource) error {
	now := time.Now().UTC().Format(time.RFC3339)
	createdAt := res.CreatedAt
	if createdAt == "" {
		if existing, err := r.GetEmbedding(res.ID); err != nil {
			return err
		} else if existing != nil {
			createdAt = existing.CreatedAt
		}
	...
	_, err := r.db.Exec(`INSERT OR REPLACE INTO embedding_resources (
		resource_id,
		name,
		base_url,
		api_key,
...
func (r *ResourceRepo) GetEmbedding(id string) (*EmbeddingResource, error) {
	...
	err := r.db.QueryRow(`SELECT
		...
		created_at,
		updated_at
		FROM embedding_resources WHERE resource_id = ?`, id).
		Scan(
			&res.ID,
			&name,
			&res.BaseURL,
...
func (r *ResourceRepo) DeleteEmbedding(id string) (bool, error) {
	res, err := r.db.Exec(`DELETE FROM embedding_resources WHERE resource_id = ?`, id)
	if err != nil {
		return false, err
	}
	n, _ := res.RowsAffected()

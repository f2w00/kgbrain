...
func (r *Repository) ensureTable(ctx context.Context) error {
	...
	if _, err := r.db.ExecContext(ctx, `CREATE INDEX IF NOT EXISTS idx_data_process_records_source
		ON data_process_records (source_table, source_key)`); err != nil {
		return fmt.Errorf("create data_process_records source index: %w", err)
	}
	if _, err := r.db.ExecContext(ctx, `CREATE INDEX IF NOT EXISTS idx_data_process_records_process
		ON data_process_records (source_table, process_type, status)`); err != nil {
		return fmt.Errorf("create data_process_records process index: %w", err)
	}
	return nil
}

// UpsertMany 批量登记处理状态；相同业务数据和处理类型会覆盖为最新状态。
...
func (r *Repository) upsertBatch(ctx context.Context, records []Record) error {
	...
	for _, record := range records {
		...
		args = append(args, record.SourceTable, record.SourceKey, record.ProcessType, record.Status)
		argIndex += 4
	}
	query := fmt.Sprintf(`INSERT INTO data_process_records (
		source_table, source_key, process_type, status
	) VALUES %s
	ON CONFLICT (source_table, source_key, process_type)
...
func (r *Repository) UpsertSourceRange(ctx context.Context, record SourceRangeRecord) error {
	...
	args := []any{record.SourceTable, record.ProcessType, record.Status}
